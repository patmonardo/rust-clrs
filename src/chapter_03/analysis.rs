    ...
    F: AsymptoticFunction,
{
    ...
    if !min_val.is_finite() || !max_val.is_finite() {
        return format!("Cannot visualize {} - function values out of range", f.name());
    }
    ...
    for i in 0..width {
        ...
        if val.is_finite() {
            let row = height - 1 - ((val - min_val) / (max_val - min_val) * (height - 1) as f64) as usize;
            if row < height {
                grid[row][i] = '*';
            }
        }
...
pub fn compare_functions(functions: &[&super::functions::FunctionWrapper], n_start: f64, n_end: f64, num_samples: usize) -> String {
    let mut result = String::new();
